...
public class UsuarioHandler extends BaseHandler {
    ...
    protected void process(HttpExchange ex) throws Exception {
        ...
        boolean tokenValido = authService.validarToken(token); // O la utilidad JWT que uses
        if (!tokenValido) {
            sendError(ex, 401, "Token inválido o expirado.");
            return;
        }

        // 2. Lógica normal del endpoint protegido si el token pasa
